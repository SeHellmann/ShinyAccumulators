...
function(input, output, session) {
  ...
  simulate_paths <- reactive({
      ...
      set.seed(ceiling(input$z*input$n_sim*input$delta_t*as.numeric(input$a)*input$v*input$tau*
                         input$max_rt*input$sv*input$sz*input$s)*5*as.numeric(Sys.time()-1635331880)+input$recalc)
      trial_parameters = data.frame(N= 1:input$n_sim,
                                    mu= rnorm(input$n_sim, input$v, input$sv),
                                    Zs = runif(input$n_sim, input$z-input$sz/2, input$z+input$sz/2)*as.numeric(input$a))
  ...
  {  
    ...
    if (input$sz > 0) {
      axis(side=2, at = (c(0,(as.numeric(input$z)-as.numeric(input$sz)/2), 
                           as.numeric(input$z), as.numeric(input$z)+as.numeric(input$sz)/2,1)*as.numeric(input$a)), 
